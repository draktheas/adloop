...
# ---------------------------------------------------------------------------
# Learning-reset warning — the single most important thing to relay before
# applying an in-place headline/description replacement.
# ---------------------------------------------------------------------------


class TestLearningResetWarning:
    def test_warning_present_when_headlines_replaced(self, config):
        result = write.update_responsive_search_ad(
            config,
            customer_id="1234567890",
            ad_id="999",
        ...
        assert warnings, "expected a warning when headlines are replaced"
        joined = " ".join(warnings).lower()
        assert "learning" in joined
        assert "policy review" in joined

    def test_warning_present_when_descriptions_replaced(self, config):
        result = write.update_responsive_search_ad(
            config,
            customer_id="1234567890",
            ad_id="999",
        ...
        assert result.get("error") is None
        warnings = result.get("warnings", [])
        assert warnings, "expected a warning when descriptions are replaced"
        assert "learning" in " ".join(warnings).lower()

    def test_warning_absent_for_url_only_update(self, config):
        result = write.update_responsive_search_ad(
            config,
            customer_id="1234567890",
            ad_id="999",
        ...
        assert result.get("error") is None
        # A URL-only edit is a true in-place change — no learning reset, so
        # no warning must be attached.
        assert "warnings" not in result

    def test_warning_absent_for_path_only_update(self, config):
        result = write.update_responsive_search_ad(
            config,
            customer_id="1234567890",
            ad_id="999",
    ...
    def test_warning_absent_for_clear_path_only_update(self, config):
        result = write.update_responsive_search_ad(
            config,
            customer_id="1234567890",
            ad_id="999",
    ...
    def test_warning_present_when_headlines_change_alongside_url(self, config):
        # Mixing a headline replacement with a URL edit still resets learning,
        # so the warning must fire.
        result = write.update_responsive_search_ad(
            config,
            customer_id="1234567890",
            ad_id="999",
...
class TestApply:
    ...
    def test_path2_only_field_mask(self, config):
        ...
        write._apply_update_rsa(
            client,
            "1234567890",
            {"ad_id": "999", "path2": "Downtown"},
        )

        op = ad_service.captured_operations[0]
        assert list(op.update_mask.paths) == ["responsive_search_ad.path2"]
        assert op.update.responsive_search_ad.path2 == "Downtown"
...
def _valid_headlines() -> list[str]:
    ...
    return ["Fast Free Shipping", "Shop the Sale Today", "Save 20% Now"]


def _valid_descriptions() -> list[str]:
    """Minimum-viable description set: 2 entries, each under 90 chars."""
    return [
        "Free shipping on every order. Shop the sale and save today.",
        "Thousands of five-star reviews. Money-back guarantee on all items.",
    ]
...
class TestHeadlineDescriptionValidation:
    ...
    def test_rejects_too_many_headlines_per_pin_slot(self, config):
        result = write.update_responsive_search_ad(
            ...
            headlines=[
                ...
                {"text": "Brand C", "pinned_field": "HEADLINE_1"},  # 3rd in slot
            ],
            descriptions=_valid_descriptions(),
        )
        assert result["error"] == "Validation failed"
    ...
    def test_headlines_only_does_not_require_descriptions(self, config):
        ...
        # List-replace semantics mean if you provide only headlines, Google
        # still enforces the 3-15 cap on the new headlines but doesn't touch
        # descriptions at all. So validation should pass here.
        result = write.update_responsive_search_ad(
            config,
            customer_id="1234567890",
            ad_id="999",
...
class TestHeadlineDescriptionPlanConstruction:
    ...
    def test_preserves_pinned_field_dict_input(self, config):
        result = write.update_responsive_search_ad(
            ...
            ad_id="999",
            headlines=[
                {"text": "Fast Free Shipping", "pinned_field": "HEADLINE_1"},
                "Shop the Sale Today",
                "Save 20% Now",
            ],
        )
    ...
    def test_headlines_field_mask_and_assets_populated(self, config):
        ...
        write._apply_update_rsa(
            ...
            "1234567890",
            {
                "ad_id": "999",
                "headlines": [
                    {"text": "Fast Free Shipping", "pinned_field": None},
                    {"text": "Shop the Sale Today", "pinned_field": None},
                    {"text": "Save 20% Now", "pinned_field": None},
                ],
            },
        )
        ...
        assert headlines[0].text == "Fast Free Shipping"
        assert headlines[1].text == "Shop the Sale Today"
        assert headlines[2].text == "Save 20% Now"
