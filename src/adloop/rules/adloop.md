...
| `get_reddit_ads` | Ads with `rejection_reason`, post URL, click URL | `ad_account_id`, `ad_group_id` / `campaign_id` (optional) |
...
| `update_reddit_ad_group` | Budget, bid, run dates, weekly `schedule`, targeting | `daily_budget` vs cap, `bid_value` vs `max_bid_increase_pct`; targeting lists REPLACE the keys you pass (pass full lists), other keys stay; keywords are brand-safety-checked and geos validated with Reddit at draft time |
| `update_reddit_ad` | Name, landing URL (`click_url`, verified reachable), comments on/off | Headline and body of a live post cannot change: for new copy use `draft_reddit_ad` and pause the old ad |
...
| `draft_reddit_ad_group` | Create an ad group (PAUSED) | `campaign_id`, `ad_group_name`, `conversion_pixel_id` (REQUIRED by Reddit; from `get_reddit_pixels`), at least one targeting list (geolocations/communities/interests/keywords), and for non-CBO campaigns `daily_budget` (or `lifetime_budget` + `end_time`), `bid_strategy` (BIDLESS, MANUAL_BIDDING, MAXIMIZE_VOLUME, TARGET_CPX), `bid_type` (CPC, CPM, CPV…); `optimization_goal` is the pixel event (PURCHASE, SIGN_UP, LEAD, PAGE_VISIT, …); optional `schedule` |
| `draft_reddit_ad` | Create a post on the profile, then the ad (PAUSED) | `ad_group_id`, `profile_id` (from `list_reddit_funding_instruments`), `headline` (≤300 chars), `click_url` (verified reachable), `post_type` IMAGE (+ public `image_url`, CTA from Reddit's fixed labels) or TEXT, `allow_comments` |
