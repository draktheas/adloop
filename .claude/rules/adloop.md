...
You have access to AdLoop MCP tools that connect Google Ads, Reddit Ads and Google Analytics (GA4) data. These rules teach you how to use them intelligently.
...
### Google Tag Manager Read Tools (all read-only)
...
| Tool | When to Use | Key Parameters |
|------|-------------|----------------|
| `analyze_page_speed` | Landing page speed audit — Lighthouse score, lab + real-user Core Web Vitals, top fixes with estimated savings. Slow pages depress Quality Score and waste paid clicks. | `url` (required, public http(s)), `strategy` ("mobile" default / "desktop") |
...
### Reddit Ads Read Tools

Reddit is a second ad platform with its own connection (own OAuth app, no developer token, no approval process). Every Reddit tool takes `ad_account_id` (falls back to `reddit.ad_account_id` in the config). The hierarchy is business → ad account → campaign → ad group → ad (post). There are no keywords-with-quality-scores and no match types: targeting is communities (subreddits), interests, keywords, geolocations, languages, gender, platforms. Money is returned in account currency (Reddit's microcurrency already divided).

| Tool | When to Use | Key Parameters |
|------|-------------|----------------|
| `list_reddit_accounts` | First-time discovery — businesses and ad accounts (currency, time zone, approval state) | (none) |
| `list_reddit_funding_instruments` | Before creating a campaign or ad — billing instruments (need `is_servable`) and posting profiles | `ad_account_id` |
| `get_reddit_campaigns` | Campaign inventory — configured vs effective status, objective, CBO flag, budget/bid | `ad_account_id`, `include_archived` |
| `get_reddit_ad_groups` | Budgets, bids, pixel, targeting and weekly schedule (`schedule_summary`, e.g. `Mon–Fri 13:00–23:59`) per ad group | `ad_account_id`, `campaign_id` (optional) |
| `get_reddit_ads` | Ads with `rejection_reason`, post URL, click URL; `include_copy=true` adds the post (type, headline, body, destination, media) so creative can be reviewed without opening Reddit | `ad_account_id`, `ad_group_id` / `campaign_id` (optional), `include_copy` |
| `get_reddit_performance` | Any Reddit performance question — spend, clicks, CTR, CPC, conversions, CPA, ROAS with names joined and `insights[]` | `level` (account/campaign/ad_group/ad), `date_range_start`, `date_range_end`, `breakdown` (date/hour/country/region/community/keyword/interest/placement/gender/os_type), `compact` |
| `run_reddit_report` | Metrics the curated tool omits (REACH, video metrics, per-event conversions) | `fields`, `breakdowns` (≤3), `filter`, date range |
| `get_reddit_pixels` | Before any conversion-optimized work — pixels and when each event last fired; flags ad groups optimizing for an event the pixel never sent | `ad_account_id` |
| `search_reddit_targeting` | Look up ids/names for ad-group targeting; `community_suggestions` returns Reddit's related-community picks for seed communities or a website | `kind` (communities/interests/geolocations/languages/keywords/community_suggestions), `query`, `country`, `website_url` |
| `get_reddit_account_history` | "What changed?" before diagnosing a performance move — who changed which field, before/after | `date_range_start`, `date_range_end`, optional `entity_type` + `entity_ids` |
| `estimate_reddit_ad_group` | Before `draft_reddit_ad_group`: targetable audience, delivery estimate, and Reddit's suggested bid range for the same targeting and budget (read-only) | `daily_budget`, `bid_type`, targeting lists, optional `bid_value` to sanity-check |

- **Status has two layers.** `configured_status` is what the advertiser set (ACTIVE/PAUSED/ARCHIVED); `effective_status` is what Reddit computes (`PENDING_APPROVAL`, `REJECTED`, `PENDING_BILLING_INFO`, `PENDING_ID_VERIFICATION`, `CAMPAIGN_PAUSED`, ...). An ACTIVE entity that is REJECTED or blocked by billing does not serve; report the effective status and its cause, not the configured one.
- **Budgets live on ad groups** unless the campaign has `is_campaign_budget_optimization: true` (then on the campaign). `goal_type` DAILY_SPEND vs LIFETIME_SPEND cannot change after publishing.
- **Report data lags up to 6 hours** and a window ending today may be empty or partial. Use `compact=true` for account audits.
- **Schedules (`schedule` on ad groups, or on CBO campaigns):** a list of blocks, `{"days": "MON-FRI", "start_hour": 13, "end_hour": 23}` or the native `{"start_day": "FRI", "start_hour": 22, "end_day": "SAT", "end_hour": 3}`; day names only (Reddit's numeric days differ between its spec and its API), `end_hour` inclusive, `[]` clears. Hours apply in each **viewer's local time**, not the account's: a 13:00–23:59 block reaches US viewers until the next morning in Europe. Zero impressions outside the schedule (a whole weekend, say) is by design; check `schedule_summary` before calling it a delivery problem.
...
1. Call `get_reddit_performance` (level `campaign`, `compact=true` for an overview); drill into `ad_group` / `ad` for the top spenders
1a. If performance moved suddenly, call `get_reddit_account_history` for the same window: a budget, bid, targeting or status change by a colleague explains most cliffs
2. Read `insights[]`: zero-conversion spenders, rejected ads, empty windows
3. If conversions are zero or implausibly low, call `get_reddit_pixels` BEFORE blaming targeting: a pixel that never fired the optimization event explains most "Reddit doesn't convert" cases
4. Cross-check `get_reddit_ads` for `REJECTED` ads (`rejection_reason`) and `PENDING_APPROVAL` — they explain missing spend
5. Present spend, clicks, CTR, CPC, conversions, CPA in account currency; compare against Google Ads only on CPA/ROAS, never on CTR (feed placements behave differently from search)
...
2. If GTM account/container IDs are unknown, call `list_gtm_accounts` → `list_gtm_containers` (or take them from the GTM UI: Admin → Container Settings)
3. Call `audit_event_coverage` with the extracted `expected_events` — it returns a per-event status matrix (ok, tag_paused, tag_active_but_not_firing, gtm_paused_but_firing, ga4_only, …) plus insights
4. Walk the non-ok statuses with the user: paused tags that should be live, active tags that never fire (trigger/filter problems — drill in with `get_gtm_trigger`), events firing with no tag or codebase reference (stale or third-party sources)
5. `gtm_paused_but_firing` means the event reaches GA4 from somewhere other than the paused tag — hardcoded gtag, another container, or a mobile SDK; don't advise unpausing without finding the real source first
6. Remember dynamic-event tags (`{{Event}}`) and Custom HTML tags are opaque to the audit — the tool lists them separately for manual review

### Planning Tools

| Tool | When to Use | Key Parameters |
|------|-------------|----------------|
| `discover_keywords` | Discover new keyword ideas from seed keywords and/or a URL — returns avg monthly searches, competition, and bid range | `seed_keywords` (list, optional), `url` (optional), `geo_target_id`, `language_id`, `page_size`, `include_monthly_volumes` (per-month history + seasonality insight — use for "when does demand peak" questions) |
| `estimate_budget` | Budget planning before launching a campaign — forecasts clicks, cost, and conversions for a set of keywords | `keywords` (list of {text, match_type, max_cpc}), `daily_budget` (optional), `geo_target_id`, `language_id`, `forecast_days` |
...
Same draft → preview → `confirm_and_apply` gate; the plan carries `platform: "reddit"` and `confirm_and_apply` routes it to Reddit. New campaigns, ad groups and ads are created **PAUSED**. Reddit has no validate-only mode, so `dry_run=true` re-reads the target entity and re-checks the safety caps (`checks` in the response); `DRY_RUN_FAILED` means the real apply would fail too.

| Tool | What It Does | Validation |
|------|-------------|------------|
| `pause_reddit_entity` / `enable_reddit_entity` | Set `configured_status` PAUSED / ACTIVE on a campaign, ad group or ad | `entity_type` (campaign/ad_group/ad), `entity_id`; enabling a new ad triggers policy review |
| `remove_reddit_entity` | ARCHIVE (irreversible — Reddit has no hard delete for entities that ran) | double confirmation; prefer pausing |
| `update_reddit_campaign` | Name, run dates, `spend_cap`; budget/bid/`schedule` only on CBO campaigns (a campaign schedule replaces every ad group's) | `daily_budget` or `lifetime_budget` (+ `end_time`) checked against `max_daily_budget`; preview shows old → new |
| `update_reddit_ad_group` | Budget, bid, run dates, weekly `schedule`, targeting incl. `locations` (placements: FEED, COMMENTS_PAGE) | `daily_budget` vs cap, `bid_value` vs `max_bid_increase_pct`; targeting lists REPLACE the keys you pass (pass full lists), other keys stay; keywords are brand-safety-checked and geos validated with Reddit at draft time |
| `update_reddit_ad` | Name, landing URL (`click_url`, verified reachable), comments on/off | Headline and body of a live post cannot change: for new copy use `draft_reddit_ad` and pause the old ad. TEXT (free-form) ads refuse a `click_url`: they open the post, the link lives in the body |
| `draft_reddit_campaign` | Create a campaign (PAUSED) | `campaign_name`, `objective` (CLICKS, CONVERSIONS, IMPRESSIONS, LEAD_GENERATION, APP_INSTALLS, CATALOG_SALES, VIDEO_VIEWABLE_IMPRESSIONS), `funding_instrument_id` (from `list_reddit_funding_instruments`); `campaign_budget_optimization=true` needs budget + `bid_strategy` + `bid_type` + `conversion_pixel_id` |
| `draft_reddit_ad_group` | Create an ad group (PAUSED) | `campaign_id`, `ad_group_name`, `conversion_pixel_id` (REQUIRED by Reddit; from `get_reddit_pixels`), at least one targeting list (geolocations/communities/interests/keywords), and for non-CBO campaigns `daily_budget` (or `lifetime_budget` + `end_time`), `bid_strategy` (BIDLESS, MANUAL_BIDDING, MAXIMIZE_VOLUME, TARGET_CPX), `bid_type` (CPC, CPM, CPV…); `optimization_goal` is the pixel event (PURCHASE, SIGN_UP, LEAD, PAGE_VISIT, …); optional `schedule`; `locations` = placements (FEED, COMMENTS_PAGE) |
| `draft_reddit_ad` | Create a post on the profile, then the ad (PAUSED) | `ad_group_id`, `profile_id` (from `list_reddit_funding_instruments`), `headline` (≤300 chars), `click_url` (verified reachable), `post_type` IMAGE (+ public `image_url`, CTA from Reddit's fixed labels) or TEXT, `allow_comments`; or `post_id` (t3_…) to promote an EXISTING post and keep its upvotes and comments (no post is created, copy args are ignored, TEXT posts take no `click_url`, media posts default it to the post's destination) |

- **Choose the post type deliberately.** An IMAGE ad clicks through to `click_url` (a normal landing-page ad). A TEXT ("free-form") ad clicks through to the **post itself**: Reddit forbids a `click_url` on it, so the landing link must be written into `body` (the tool refuses otherwise) and the report shows post opens and comments, not site clicks. Use TEXT for conversation-style founder posts, IMAGE when you want landing-page traffic. Neither can be edited after creation except `allow_comments`; new copy means a new post and ad.
- **Ad groups need a schedule.** Reddit requires `start_time`; the tool defaults it to now. Pass `end_time` too when the budget is `lifetime_budget`.
...
**Safety behaviors:**
- New campaigns and RSAs (and every Reddit campaign, ad group and ad) are created as PAUSED — user must explicitly enable them after review.
...
7. After campaign creation, remind the user to:
   ...
   - **If a shared negative keyword list already exists** for the account (check via `get_negative_keyword_lists`), call `attach_shared_set_to_campaigns` so the new campaign inherits historical waste-blockers. New campaigns don't auto-inherit shared lists — this step is easy to forget.
...
4. Present the forecast: estimated clicks, cost, avg CPC, and conversions
...
8. **Attaching an existing shared list to new campaigns**: use `attach_shared_set_to_campaigns` with `shared_set_id` from `get_negative_keyword_lists` and the list of `campaign_ids` you want covered. Most common right after `draft_campaign` — newly-built campaigns don't auto-inherit shared lists. Use `get_negative_keyword_list_campaigns` first to skip campaigns that already have the linkage. Use `detach_shared_set_from_campaigns` to undo. Both go through the standard draft → preview → `confirm_and_apply` flow.
...
### When user wants a Reddit campaign (or ad group / ad)

Reddit's structure is campaign → ad group (budget, bid, pixel, targeting) → ad (a post). Build it in that order, one PAUSED entity at a time, each through preview → approval → `confirm_and_apply`.

1. `get_reddit_campaigns` to avoid duplicate names and to learn the account's currency; `list_reddit_funding_instruments` for a servable `funding_instrument_id` and the posting `profile_id`
2. **Pixel first.** `get_reddit_pixels`: every ad group needs a `conversion_pixel_id`, and a CONVERSIONS objective is pointless if the optimization event never fired. If the account has no pixel, stop and tell the user to create one in Reddit Events Manager
3. **Targeting.** Ask which countries (and languages, upper-case codes like `EN`) to target if not stated; find ids/names with `search_reddit_targeting` (`geolocations` with `country`, `communities` by query, `community_suggestions` for related subreddits, `interests`, `keywords`). Never draft an untargeted ad group; warn when a geo-targeted ad group has no language list
3a. **Estimate first.** `estimate_reddit_ad_group` with the same targeting and budget: if the targetable audience is tiny or the planned bid is under Reddit's suggested minimum, fix that before drafting
4. `draft_reddit_campaign` (objective, funding instrument; leave the budget off unless the user wants campaign budget optimization) → approve → apply
5. `draft_reddit_ad_group` with `daily_budget` ≤ `max_daily_budget`, `bid_strategy` (MAXIMIZE_VOLUME is the sane default; MANUAL_BIDDING/TARGET_CPX need `bid_value`), `bid_type` (CPC for traffic and conversions), `conversion_pixel_id`, `optimization_goal` matching the pixel event, targeting → approve → apply
6. `draft_reddit_ad`: verify `click_url` exists (route definition or live page), write a headline in the landing page's language (Reddit titles read like posts, not like search ads — no ALL CAPS, no exclamation stacking), pick a `call_to_action` from Reddit's list, decide `allow_comments` with the user (comments are public and need moderation) → approve → apply
7. Enable in reverse order once everything exists: ad (`enable_reddit_entity`, triggers policy review; expect `PENDING_APPROVAL` for up to 24h), then ad group, then campaign
8. After 24h check `get_reddit_ads` for `REJECTED` + `rejection_reason` before touching budgets
...
2. Call `pause_entity` or `enable_entity` (Google) or `pause_reddit_entity` / `enable_reddit_entity` (Reddit) with the entity type and ID
...
**Audience type ↔ campaign type compatibility.** Not every audience type can be attached to every campaign type — check this BEFORE proposing audience targeting, and use it to explain empty `custom_audience` joins:

| Audience type | Search | Display | Video (YouTube) | Performance Max |
|---|---|---|---|---|
| Remarketing / your-data lists | ✓ (targeting or observation) | ✓ | ✓ | signal only |
| In-market segments | ✓ | ✓ | ✓ | signal only |
| Affinity segments | ✓ | ✓ | ✓ | signal only |
| Custom segments (custom intent/affinity) | ✗ NOT attachable | ✓ | ✓ | signal only |
| Combined segments | ✓ | ✗ | ✗ | ✗ |
| Detailed demographics | ✓ | ✓ | ✓ | signal only |

- **Custom segments cannot be attached to Search campaigns.** A GAQL join of `custom_audience` against Search campaigns will always come back empty — that is expected, not missing data. Don't propose custom segments for Search; suggest in-market/affinity or remarketing lists instead.
- **PMax never takes hard audience targeting.** Every audience there is an asset-group-level signal — a hint to the algorithm, not a filter.
